  ...
  return (
    <FadeRight>
      ...
      >
        ...
        <StaggerContainer>
          <ul className="space-y-2.5" aria-labelledby="skills-working-style-heading">
            {workingStyle.items.map((item) => (
              <StaggerItem key={item.label}>
                <li className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3 transition-all duration-300 hover:border-[#ef9d7a]/30 hover:bg-white/[0.04]">
                  <span
                    aria-hidden="true"
                    className="size-1.5 shrink-0 rounded-full bg-[#ef9d7a]"
                  />
                  <span className="font-poppins text-sm font-medium text-text/85">
                    {item.label}
                  </span>
                </li>
              </StaggerItem>
