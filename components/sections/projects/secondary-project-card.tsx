...
export function SecondaryProjectCard({ project }: SecondaryProjectCardProps) {
  ...
  return (
    <StaggerItem className="h-full">
      <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/2 transition-all duration-500 hover:-translate-y-1 hover:border-[#8852F7]/30 hover:shadow-[0_0_30px_rgba(135,80,247,0.15)] active:-translate-y-1 active:border-[#8852F7]/30 active:shadow-[0_0_30px_rgba(135,80,247,0.15)] shadow-lg">
        {/* Subtle overlay card background gradient */}
        <div className="absolute inset-0 -z-10 bg-linear-to-tr from-[#8852F7]/[0.01] via-transparent to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100 group-active:opacity-100" />
        ...
        <div className="flex flex-1 flex-col justify-between p-5 sm:p-6 gap-6">
            ...
            <ul className="flex flex-wrap gap-1.5" aria-label="Technologies used">
              {project.techStack.map((tech) => (
                <li key={tech}>
                  <span
                    tabIndex={0}
                    className={`inline-flex items-center gap-1 rounded-lg border border-white/5 bg-white/1 px-2 py-0.5 font-poppins text-[10px] font-medium text-text/60 hover:text-text hover:border-[#8852F7]/30 hover:bg-[#8852F7]/5 active:text-text active:border-[#8852F7]/30 active:bg-[#8852F7]/5 transition-colors duration-300 ${focusRing}`}
                  >
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-white/4">
            <a
              ...
              className={`inline-flex min-h-9 cursor-pointer items-center justify-center gap-1.5 rounded-lg bg-primary px-4 py-2 font-poppins text-[11px] font-semibold text-white shadow-md shadow-primary/10 transition-all duration-300 hover:bg-primary/90 hover:shadow-primary/20 active:scale-[0.98] ${focusRing}`}
            >
            ...
            {project.links.isPrivate ? (
              <button
                type="button"
                disabled
                className="inline-flex min-h-9 cursor-not-allowed items-center justify-center gap-1.5 rounded-lg border border-white/5 bg-white/2 px-4 py-2 font-poppins text-[11px] font-semibold text-text/30"
